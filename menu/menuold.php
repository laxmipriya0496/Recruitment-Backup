  ...
  <div class="container">
	  <div class="row">
		  <div class="inner-container">
		  	...
		  	<?php
				$menu_master=mssql_query("select distinct m.menu_name,m.id,m.m_class from menu_access_right a join menu_master m
on m.id=a.menu_id 
 join user_master u on a.user_id=u.user_id
where u.user_id='$user_id' order by m.id");
/* echo "select distinct m.menu_name,m.id,m.m_class from menu_access_right a join menu_master m
on m.id=a.menu_id 
 join user_master u on a.user_id=u.user_id
where u.user_id='$user_id' order by m.id"; */
$s=1; 
			...
			<div class="col-md-4" id="hide<?php echo $s; ?>" onclick="iconhide('#hide<?php echo $s; ?>','#show<?php echo $s; ?>');check('<?php echo $s; ?>')">
			 ...
			 <div class="col-md-4" id="show<?php echo $s++;  ?>" onclick="iconshow()">
				<nav class="navbar">
				  <div class="container-fluid">
					...
					<div class="sub_menu">
						<li class="dropdown1">
									 ...
									 <?php
			 $submenu_master=mssql_query("select distinct s.submenu_code,s.submenu_order,s.submenu_class,s.submenu_url from menu_access_right a join user_master u on a.user_id=u.user_id join submenu_master s on s.id=a.submenu_id
where u.user_id='$user_id' and s.menu_id='$subid' order by s.submenu_order");

/* echo "select distinct s.submenu_code,s.submenu_order,s.submenu_class,s.submenu_url from menu_access_right a join user_master u on a.user_id=u.user_id join submenu_master s on s.id=a.submenu_id
where u.user_id='$user_id' and s.menu_id='$subid' order by s.submenu_order"; */
