...
<?php
	$reg=mssql_fetch_array(mssql_query("select user_name,full_name,user_id from user_master where user_id='$id'"));
?>
                  ...
                  <div class="box-body">
                   <table class="table table-hover" style="font-family:'Times New Roman', Times, serif">
						<tr>
							...
							<option value="<?php  echo $reg['user_name']."-".$reg['full_name']; ?>"><?php  echo $reg['user_name']."-".$reg['full_name']; ?></option>
				<?php
				$usergroup1=mssql_query("select user_name,full_name,user_id from user_master where user_name not in (select user_id from role_mapping) and user_id!='$id'");
    ...
    <select id="multiple-checkboxes" multiple="multiple" name="department_name[]" class="form-control"  style="width:100%;" required/>
        <option value="All">All</option>
						<?php 
						$uni=mssql_query("select distinct Department from employee_master ");
						while($uni1=mssql_fetch_array($uni))
						{
						?>
							<option value="<?php echo $uni1['Department']; ?>"><?php echo $uni1['Department']; ?></option>
						<?php
						}
						?>
    </select>
 ...
 if(isset($_REQUEST['formsent']))
{
	...
	if($department_name=="All")
	{
		...
		$n=mssql_query("update department_mapping set department='$department_name',modified_by='$user',modified_on='$date' where user_id='$id'");
if($n)
	{?>
	<script>
    setTimeout(function() {
