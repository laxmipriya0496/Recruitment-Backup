...
<div class="wrapper" style=" width: 97%; margin:0 auto;">
	...
	<ul class="block-menu">
		...
		<li><a href="department_master.php" class="three-d">Department
			<span aria-hidden="true" class="three-d-box">
				<span class="front" style="background-color:#3db2e1;">Department</span>
...
<!-- more items here -->
			...
			<?php
				$uni1=mssql_fetch_array(mssql_query("select * from department_master where id='$id'"));
			?>
			<div class="col-md-4">
                  ...
                  <div class="box-body">
                   <table class="table table-hover" style="font-family:'Times New Roman', Times, serif">
						<tr>
							...
							<td><input type="text" class="form-control" name="department_code" id="department_code" readonly value="<?php echo $uni1['department_code']; ?>" required></td>
						</tr>
						<tr>
							<td>Department Name</td>
							<td><input type="text" class="form-control" name="department_name" id="department_name" value="<?php echo $uni1['department_name']; ?>" required></td>
						</tr>				
						<tr>
							<td>Status</td>
			...
			<form role="form" name="area"  method="post">
                  <div class="box-body">
                   <table class="table table-hover" style="font-family:'Times New Roman', Times, serif;margin-bottom:50px;">
						...
						<?php $uni=mssql_query("select *,case when (status=0) then 'Active' else 'Inactive' end as sta from department_master order by department_code asc");
				...
				?>
						...
						<td  align="center"><div style=" white-space: nowrap;"><a href="department_masteredit.php?id=<?php echo $row['id'];?>" class="btn btn-warning btn-sm"><i class="fa fa-pencil fa-lg icon-white"></i></a></div></td>
						</tr>	
				<?php
					}
					?>
...
{  
	...
	$old_det=mssql_fetch_array(mssql_query("select * from department_master where id='$id'"));
	$oid_department_name=$old_det['department_name'];
	$oid_status=$old_det['status'];
	
	$old=array($oid_department_name,$oid_status);
	
	$new=array($department_name,$status);
	
	$field=array("department_name","status");
	...
	for($r=0;$r<$tottt;$r++)
	{
		if($old[$r]!=$new[$r])
		{
			...
			$ins=mssql_query("INSERT INTO updated_data(employee_id,column_name,old_value,new_value,master_name,ip_address,created_by,created_on) VALUES ('$department_code','$field[$r]','$old[$r]','$new[$r]','Department Master','$ip','$user','$d')");
	...
	$n=mssql_query("update department_master set department_name='$department_name',status='$status',modified_by='$user',modified_on='$date' where id='$id'");
if($n)
	{
	?>
	<script>
    setTimeout(function() {
        swal({
            ...
            window.location = "department_master.php";
        });
    }, 100);
</script>  
    <?php
