  ...
  <body class="skin-blue fixed sidebar-mini">
	...
	else
	{
		$dt="";
				...
				$inndate1=mssql_query("select Department,COUNT(Department) as tot from employee_master $dt
...
$s=1;
		while($c=mssql_fetch_array($inndate1))
		{
				...
				<td>
				<a href="depdetails.php?dep=<?php echo $c['Department']; ?>" target="_blank"><?php echo $c['tot']; ?></a></td>
							</tr>
						<?php
			   }
			   ?>
								<?php
								$state=mssql_query("select Department,count(Department) as tot from employee_master $dt group by Department");
								$dataPoints1='';
								$count=mssql_num_rows($state);
								$i=1;
								while($state_res=mssql_fetch_array($state)){	
								$dataPoints1.='{"y":'.$state_res['tot'].',"name":"'.$state_res['Department'].'"}';
								if($i<$count){
								$dataPoints1.=',';
								}$i++;
								}
            ...
            $(function () {
                ...
                chart.render();
				function onClick(e) {
		window.open("depdetails.php?dep=" + e.dataPoint.name, '_blank');
	}
            });
        </script>
				</table>
