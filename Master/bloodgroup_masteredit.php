...
<!-- more items here -->
			...
			<?php
				$uni1=mssql_fetch_array(mssql_query("select * from bloodgroup_master where id='$id'"));
			?>
			<div class="col-md-4">
				 <form role="form" name="area"  method="post">
                  <div class="box-body">
                   <table class="table table-hover" style="font-family:'Times New Roman', Times, serif">
						<tr>
							<td>Blood_Group Code</td>
							<td><input type="text" class="form-control" name="bloodgroup_code" id="bloodgroup_code" readonly value="<?php echo $uni1['bloodgroup_code']; ?>" required></td>
						</tr>
						<tr>
							<td>Blood_Group Name</td>
							<td><input type="text" class="form-control" name="bloodgroup_name" id="bloodgroup_name" value="<?php echo $uni1['bloodgroup_name']; ?>" required></td>
						</tr>				
						<tr>
							<td>Status</td>
...
{  
	...
	$old_det=mssql_fetch_array(mssql_query("select * from bloodgroup_master where id='$id'"));
	$oid_bloodgroup_name=$old_det['bloodgroup_name'];
	$oid_status=$old_det['status'];
	
	$old=array($oid_bloodgroup_name,$oid_status);
	
	$new=array($bloodgroup_name,$status);
	
	$field=array("bloodgroup_name","status");
	...
	for($r=0;$r<$tottt;$r++)
	{
		if($old[$r]!=$new[$r])
		{
			...
			$ins=mssql_query("INSERT INTO updated_data(employee_id,column_name,old_value,new_value,master_name,ip_address,created_by,created_on) VALUES ('$bloodgroup_code','$field[$r]','$old[$r]','$new[$r]','Blood Group Master','$ip','$user','$d')");
	...
	$rs=mssql_fetch_array(mssql_query("select * from bloodgroup_master where id='$id'"));
	$old_un=$rs['bloodgroup_name'];
	
	$n=mssql_query("update bloodgroup_master set bloodgroup_name='$bloodgroup_name',status='$status',modified_by='$user',modified_on='$date' where id='$id'");
	
	$n1=mssql_query("update employee_master set Blood_Group='$bloodgroup_name' where Blood_Group='$old_un'");
if($n)
	{
	?>
	<script>
