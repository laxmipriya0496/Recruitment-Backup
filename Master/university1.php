...
input[type='text'] {
          ...
          <div class="box">
				...
				<form method="post">
				 <table class="table table-hover" style="font-family:'Times New Roman', Times, serif">
					...
					<td colspan=5>
					<select class="selectpicker" data-show-subtext="true" data-live-search="true" name="university_name1[]" id="university_name1" class="form-control" value="" autocomplete="off" multiple  onchange="get_gra(this.value)">
 ...
 where m.Department in ('$dep')  and d.University like '%University%'");
...
$cus=mssql_query("select *,m.id as eid from employee_details d join employee_master m on d.emp_id=m.employee_id
 where d.University in('$uni') and m.Department in('$dep') $grd $cat $dep1 order by employee_id");
...
$(".content-wrapper1").click(function(){
	var select_value=$(".btn.dropdown-toggle.selectpicker.btn-default").attr('title');
	//alert(select_value);
		 $.ajax
		({
			type: "GET",
			url: "get_uni.php",
			data: "uni=" + select_value + "&dep=<?php echo $dep; ?>",		 
			success: function(data)
			{
